...
export function useFadeUp() {
  ...
  return function fadeUp(delay = 0): MotionProps {
    ...
    // Switched from `whileInView` → `animate` so the fade-up triggers on
    // mount instead of needing a real scroll into view. Avoids the
    // "section appears empty in screenshots / preview captures" trap
    // (Playwright `fullPage` snapshots before scroll, OG image renders,
    // SSR-snapshot bots, etc.). Also more forgiving on fast scrolls.
    return {

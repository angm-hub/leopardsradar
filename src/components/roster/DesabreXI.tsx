/**
 * DesabreXI — 11 preferentiel Leopards · Composition historique reelle.
 *
 * Source principale (depuis scrape_rdc_lineups.py) :
 *   Vue materielle desabre_xi_stats, alimentee par national_lineups.
 *   Affiche le joueur le plus souvent titularise par poste sous Desabre
 *   depuis le 07/08/2022 (date de prise de fonction, source : Wikipedia + TM).
 *
 * Fallback (si la vue est vide = pas encore de scrape) :
 *   Algo proxy : level_score + market_value_eur + caps_rdc (ancien comportement).
 *   Un disclaimer clair distingue les deux modes.
 *
 * Terrain : SVG sobre, fond graphite, lignes blanches a 15% opacite.
 * Formation 4-3-3 par defaut, adaptee a la formation detectee dans les matchs.
 */

import { useMemo, useState } from "react";
import { Link } from "react-router-dom";
import { cn } from "@/lib/utils";
import { PlayerAvatar } from "@/components/ui/PlayerAvatar";
import { POSITION_LABEL } from "@/lib/playerHelpers";
import { useDesabreXI, type DesabreSlot } from "@/hooks/useDesabreXI";
import type { DBPlayer, DBPosition } from "@/types/dbPlayer";

// ─── Types communs ────────────────────────────────────────────────────────────

type FormationSlot = {
  pos: DBPosition;
  x: number;   // % du SVG (0=gauche, 100=droite)
  y: number;   // % du SVG (0=haut=attaque, 100=bas=GK)
  label: string;
};

// Formation 4-3-3 — Y inversé : attaquants en haut (20%), GK en bas (90%)
const FORMATION_433: FormationSlot[] = [
  { pos: "Goalkeeper", x: 50, y: 90, label: "GK" },
  { pos: "Defender",   x: 15, y: 73, label: "RB" },
  { pos: "Defender",   x: 35, y: 73, label: "CB" },
  { pos: "Defender",   x: 65, y: 73, label: "CB" },
  { pos: "Defender",   x: 85, y: 73, label: "LB" },
  { pos: "Midfield",   x: 25, y: 52, label: "MF" },
  { pos: "Midfield",   x: 50, y: 52, label: "MF" },
  { pos: "Midfield",   x: 75, y: 52, label: "MF" },
  { pos: "Attack",     x: 20, y: 28, label: "RW" },
  { pos: "Attack",     x: 50, y: 22, label: "ST" },
  { pos: "Attack",     x: 80, y: 28, label: "LW" },
];

// ─── Terrain SVG ─────────────────────────────────────────────────────────────

function PitchSVG() {
  const stroke = "rgba(255,255,255,0.14)";
  const sw = 1;
  return (
    <svg
      viewBox="0 0 100 120"
      preserveAspectRatio="xMidYMid meet"
      aria-hidden
      className="absolute inset-0 w-full h-full"
    >
      <rect width="100" height="120" fill="transparent" />
      <rect x="3" y="3" width="94" height="114" rx="1" fill="none" stroke={stroke} strokeWidth={sw} />
      <line x1="3" y1="60" x2="97" y2="60" stroke={stroke} strokeWidth={sw} />
      <circle cx="50" cy="60" r="12" fill="none" stroke={stroke} strokeWidth={sw} />
      <circle cx="50" cy="60" r="1.2" fill={stroke} />
      <rect x="23" y="3" width="54" height="18" fill="none" stroke={stroke} strokeWidth={sw} />
      <rect x="34" y="3" width="32" height="7" fill="none" stroke={stroke} strokeWidth={sw} />
      <path d="M 30 21 A 12 12 0 0 1 70 21" fill="none" stroke={stroke} strokeWidth={sw} />
      <rect x="23" y="99" width="54" height="18" fill="none" stroke={stroke} strokeWidth={sw} />
      <rect x="34" y="110" width="32" height="7" fill="none" stroke={stroke} strokeWidth={sw} />
      <path d="M 30 99 A 12 12 0 0 0 70 99" fill="none" stroke={stroke} strokeWidth={sw} />
      <circle cx="50" cy="13" r="1" fill={stroke} />
      <circle cx="50" cy="107" r="1" fill={stroke} />
      <path d="M 3 3 A 3 3 0 0 1 6 6" fill="none" stroke={stroke} strokeWidth={sw} />
      <path d="M 97 3 A 3 3 0 0 0 94 6" fill="none" stroke={stroke} strokeWidth={sw} />
      <path d="M 3 117 A 3 3 0 0 0 6 114" fill="none" stroke={stroke} strokeWidth={sw} />
      <path d="M 97 117 A 3 3 0 0 1 94 114" fill="none" stroke={stroke} strokeWidth={sw} />
    </svg>
  );
}

// ─── Pin joueur (mode reel) ───────────────────────────────────────────────────

function RealPlayerPin({
  slot_def,
  player_slot,
  players,
}: {
  slot_def: FormationSlot;
  player_slot: DesabreSlot;
  players: DBPlayer[];
}) {
  // On essaie de trouver le joueur dans la liste players pour avoir l'image
  const dbPlayer = players.find((p) => p.id === player_slot.player_id);

  return (
    <Link
      to={`/player/${player_slot.slug}`}
      className={cn(
        "absolute flex flex-col items-center gap-1 -translate-x-1/2 -translate-y-1/2",
        "group focus-visible:outline-none",
      )}
      style={{ left: `${slot_def.x}%`, top: `${slot_def.y}%` }}
      aria-label={`${player_slot.player_name} — ${POSITION_LABEL[slot_def.pos as DBPosition] ?? slot_def.label} — ${player_slot.appearances_total} titularisations`}
    >
      <div className="relative">
        <PlayerAvatar
          name={player_slot.player_name}
          src={dbPlayer?.image_url ?? null}
          srcAlt={dbPlayer?.image_url_alt ?? null}
          className="h-10 w-10 sm:h-12 sm:w-12 rounded-full ring-2 ring-primary/40 group-hover:ring-primary transition-all duration-200 shadow-md shadow-black/60"
          initialsClassName="text-xs"
        />
        <span className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-background/90 border border-border px-1.5 py-px text-[8px] font-mono uppercase tracking-wider text-muted-light shadow-sm">
          {slot_def.label}
        </span>
      </div>
      <div className="flex flex-col items-center gap-0">
        <p className="text-center font-serif text-[9px] sm:text-[10px] font-semibold text-foreground leading-tight max-w-[60px] sm:max-w-[72px] group-hover:text-primary transition-colors drop-shadow-md line-clamp-2">
          {player_slot.player_name.split(" ").at(-1)}
        </p>
        {/* Badge frequence */}
        <span className="font-mono text-[8px] text-primary/70 tabular-nums">
          {player_slot.start_pct}%
        </span>
      </div>
    </Link>
  );
}

// ─── Pin joueur (mode fallback algo) ─────────────────────────────────────────

function AlgoPlayerPin({
  player,
  slot,
}: {
  player: DBPlayer;
  slot: FormationSlot;
}) {
  return (
    <Link
      to={`/player/${player.slug}`}
      className={cn(
        "absolute flex flex-col items-center gap-1 -translate-x-1/2 -translate-y-1/2",
        "group focus-visible:outline-none",
      )}
      style={{ left: `${slot.x}%`, top: `${slot.y}%` }}
      aria-label={`${player.name} — ${POSITION_LABEL[slot.pos] ?? slot.label}`}
    >
      <div className="relative">
        <PlayerAvatar
          name={player.name}
          src={player.image_url}
          srcAlt={player.image_url_alt}
          className="h-10 w-10 sm:h-12 sm:w-12 rounded-full ring-2 ring-primary/40 group-hover:ring-primary transition-all duration-200 shadow-md shadow-black/60"
          initialsClassName="text-xs"
        />
        <span className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-background/90 border border-border px-1.5 py-px text-[8px] font-mono uppercase tracking-wider text-muted-light shadow-sm">
          {slot.label}
        </span>
      </div>
      <p className="text-center font-serif text-[9px] sm:text-[10px] font-semibold text-foreground leading-tight max-w-[60px] sm:max-w-[72px] group-hover:text-primary transition-colors drop-shadow-md line-clamp-2">
        {player.name.split(" ").at(-1)}
      </p>
    </Link>
  );
}

// ─── Vue grille ───────────────────────────────────────────────────────────────

function RealGridView({
  xi,
  players,
}: {
  xi: DesabreSlot[];
  players: DBPlayer[];
}) {
  const byPos: Partial<Record<DBPosition, DesabreSlot[]>> = {};
  xi.forEach((slot) => {
    const pos = (slot.position ?? "Unknown") as DBPosition;
    if (!byPos[pos]) byPos[pos] = [];
    byPos[pos]!.push(slot);
  });

  const order: DBPosition[] = ["Goalkeeper", "Defender", "Midfield", "Attack"];
  return (
    <div className="space-y-4">
      {order.map((pos) => {
        const items = byPos[pos] ?? [];
        if (items.length === 0) return null;
        return (
          <div key={pos}>
            <p className="mb-2 font-mono text-[10px] uppercase tracking-[0.2em] text-muted">
              {POSITION_LABEL[pos]}
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {items.map((slot) => {
                const dbPlayer = players.find((p) => p.id === slot.player_id);
                return (
                  <Link
                    key={slot.player_id}
                    to={`/player/${slot.slug}`}
                    className={cn(
                      "flex items-center gap-2 p-2 rounded-card border border-border bg-card",
                      "hover:border-border-hover transition-colors",
                      "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary",
                    )}
                  >
                    <PlayerAvatar
                      name={slot.player_name}
                      src={dbPlayer?.image_url ?? null}
                      srcAlt={dbPlayer?.image_url_alt ?? null}
                      className="h-8 w-8 rounded-full ring-1 ring-border shrink-0"
                      initialsClassName="text-[10px]"
                    />
                    <div className="min-w-0 flex-1">
                      <p className="font-serif text-xs font-semibold text-foreground truncate leading-tight">
                        {slot.player_name}
                      </p>
                      <p className="font-mono text-[9px] text-muted-light truncate">
                        {slot.appearances_total} starts ({slot.start_pct}%)
                      </p>
                    </div>
                  </Link>
                );
              })}
            </div>
          </div>
        );
      })}
    </div>
  );
}

// ─── Fallback algo ────────────────────────────────────────────────────────────

const POSITION_COUNT: Record<DBPosition, number> = {
  Goalkeeper: 1,
  Defender: 4,
  Midfield: 3,
  Attack: 3,
};

function selectAlgoXI(players: DBPlayer[]): Array<{ player: DBPlayer; slot: FormationSlot }> {
  const roster = players.filter(
    (p) => p.player_category === "roster" && p.eligibility_status !== "ineligible",
  );
  const selected: Array<{ player: DBPlayer; slot: FormationSlot }> = [];
  const usedIds = new Set<number>();
  const positionsOrdered: DBPosition[] = ["Goalkeeper", "Defender", "Midfield", "Attack"];

  for (const pos of positionsOrdered) {
    const count = POSITION_COUNT[pos];
    const pool = roster
      .filter((p) => p.position === pos && !usedIds.has(p.id))
      .sort((a, b) => {
        const scoreA = a.level_score ?? null;
        const scoreB = b.level_score ?? null;
        if (scoreA !== null && scoreB !== null && scoreB !== scoreA) return scoreB - scoreA;
        if (scoreA !== null && scoreB === null) return -1;
        if (scoreA === null && scoreB !== null) return 1;
        const valA = a.market_value_eur ?? 0;
        const valB = b.market_value_eur ?? 0;
        if (valB !== valA) return valB - valA;
        return (b.caps_rdc ?? 0) - (a.caps_rdc ?? 0);
      });

    const slots = FORMATION_433.filter((s) => s.pos === pos);
    pool.slice(0, count).forEach((player, idx) => {
      usedIds.add(player.id);
      selected.push({ player, slot: slots[idx] ?? slots[0] });
    });
  }
  return selected;
}

function AlgoGridView({
  xi,
}: {
  xi: Array<{ player: DBPlayer; slot: FormationSlot }>;
}) {
  const byPos: Partial<Record<DBPosition, Array<{ player: DBPlayer; slot: FormationSlot }>>> = {};
  xi.forEach((item) => {
    if (!byPos[item.slot.pos]) byPos[item.slot.pos] = [];
    byPos[item.slot.pos]!.push(item);
  });
  const order: DBPosition[] = ["Goalkeeper", "Defender", "Midfield", "Attack"];
  return (
    <div className="space-y-4">
      {order.map((pos) => {
        const items = byPos[pos] ?? [];
        if (items.length === 0) return null;
        return (
          <div key={pos}>
            <p className="mb-2 font-mono text-[10px] uppercase tracking-[0.2em] text-muted">
              {POSITION_LABEL[pos]}
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {items.map(({ player }) => (
                <Link
                  key={player.id}
                  to={`/player/${player.slug}`}
                  className={cn(
                    "flex items-center gap-2 p-2 rounded-card border border-border bg-card",
                    "hover:border-border-hover transition-colors",
                    "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary",
                  )}
                >
                  <PlayerAvatar
                    name={player.name}
                    src={player.image_url}
                    srcAlt={player.image_url_alt}
                    className="h-8 w-8 rounded-full ring-1 ring-border shrink-0"
                    initialsClassName="text-[10px]"
                  />
                  <div className="min-w-0">
                    <p className="font-serif text-xs font-semibold text-foreground truncate leading-tight">
                      {player.name}
                    </p>
                    <p className="font-mono text-[9px] text-muted-light truncate">
                      {player.current_club ?? "—"}
                    </p>
                  </div>
                </Link>
              ))}
            </div>
          </div>
        );
      })}
    </div>
  );
}

// ─── Composant principal ──────────────────────────────────────────────────────

type ViewMode = "terrain" | "grille";

interface DesabreXIProps {
  players: DBPlayer[];
}

export function DesabreXI({ players }: DesabreXIProps) {
  const [viewMode, setViewMode] = useState<ViewMode>("terrain");
  const { data: realData, loading } = useDesabreXI();

  // Mode reel : on utilise desabre_xi_stats
  const useRealData = !loading && realData !== null && realData.xi.length >= 11;

  // Mode algo (fallback)
  const algoXI = useMemo(() => {
    if (useRealData) return [];
    return selectAlgoXI(players);
  }, [useRealData, players]);

  const isReady = useRealData
    ? realData!.xi.length >= 11
    : algoXI.length >= 11;

  if (loading) {
    return (
      <section className="mb-10">
        <div className="h-8 w-48 rounded bg-card animate-pulse mb-4" />
        <div className="h-64 rounded-card border border-border bg-card animate-pulse" />
      </section>
    );
  }

  if (!isReady) return null;

  const formation = useRealData ? (realData!.formation ?? "4-3-3") : "4-3-3";
  const totalMatches = useRealData ? realData!.total_matches : null;

  return (
    <section aria-labelledby="desabre-xi-heading" className="mb-10">
      {/* En-tete + toggle */}
      <div className="mb-5 flex items-start justify-between gap-4 flex-wrap">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-muted">
            Composition · {formation}
            {useRealData && totalMatches ? (
              <span className="ml-2 text-success/70">· {totalMatches} matchs</span>
            ) : null}
          </p>
          <h2
            id="desabre-xi-heading"
            className="mt-1.5 font-serif text-2xl font-semibold tracking-tight text-foreground"
          >
            11 de Desabre
          </h2>
          <p className="mt-1 max-w-lg text-xs text-muted-light leading-snug">
            {useRealData
              ? "Composition basee sur les titularisations reelles depuis aout 2022. Le joueur le plus souvent titularise par poste."
              : "Composition algorithmique basee sur le score Leopards + caps RDC. Les données historiques ne sont pas encore disponibles."}
          </p>
        </div>

        {/* Source badge */}
        <div className="flex flex-col items-end gap-2">
          {useRealData ? (
            <span className="rounded-full border border-success/30 bg-success/10 px-2.5 py-1 font-mono text-[9px] uppercase tracking-wider text-success/80">
              Historique reel
            </span>
          ) : (
            <span className="rounded-full border border-warning/30 bg-warning/10 px-2.5 py-1 font-mono text-[9px] uppercase tracking-wider text-warning/80">
              Algo proxy
            </span>
          )}
          {/* Toggle terrain / grille */}
          <div
            role="group"
            aria-label="Mode d'affichage"
            className="flex rounded-full border border-border bg-card p-0.5 gap-0.5"
          >
            {(["terrain", "grille"] as ViewMode[]).map((v) => (
              <button
                key={v}
                type="button"
                onClick={() => setViewMode(v)}
                aria-pressed={viewMode === v}
                className={cn(
                  "rounded-full px-4 py-1.5 font-mono text-[10px] uppercase tracking-wider transition-colors",
                  viewMode === v
                    ? "bg-foreground/10 text-foreground"
                    : "text-muted hover:text-muted-light",
                )}
              >
                {v === "terrain" ? "Terrain" : "Grille"}
              </button>
            ))}
          </div>
        </div>
      </div>

      {/* Terrain */}
      {viewMode === "terrain" ? (
        <div
          className="relative w-full overflow-hidden rounded-card border border-border bg-[#0d1117]"
          style={{ paddingBottom: "min(120%, 520px)" }}
          aria-label="Terrain de football avec le 11 de Desabre"
        >
          <div className="absolute inset-0">
            <PitchSVG />
            {useRealData
              ? realData!.xi.map((slot, i) => {
                  const slot_def = FORMATION_433[i] ?? FORMATION_433[0];
                  return (
                    <RealPlayerPin
                      key={slot.player_id}
                      slot_def={slot_def}
                      player_slot={slot}
                      players={players}
                    />
                  );
                })
              : algoXI.map(({ player, slot }) => (
                  <AlgoPlayerPin key={player.id} player={player} slot={slot} />
                ))}
          </div>
        </div>
      ) : (
        /* Grille */
        useRealData ? (
          <RealGridView xi={realData!.xi} players={players} />
        ) : (
          <AlgoGridView xi={algoXI} />
        )
      )}

      {/* Legende */}
      <p className="mt-3 font-mono text-[10px] text-muted">
        {useRealData
          ? `Source : Transfermarkt · compositions depuis le 07/08/2022 · % = frequence de titularisation`
          : "Algo : score Leopards (level_score) puis valeur marchande Transfermarkt, par poste."}
      </p>
    </section>
  );
}

export default DesabreXI;

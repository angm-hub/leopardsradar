 ...
 * Layout éditorial 2 colonnes : cover à droite (photo ou couverture typo
 * cobalt), texte à gauche. Sur mobile la cover passe en tête. Donne au
 * « À la une » un vrai poids visuel (audit lisibilité 11/08/2026).
 */
export function StoryHero({ story }: StoryHeroProps) {
  return (
    <Link
      to={`/histoires/${story.slug}`}
      className="group block overflow-hidden rounded-card border border-primary/30 bg-gradient-to-br from-primary/10 via-card to-background transition-all hover:border-primary/60 hover:shadow-2xl hover:shadow-primary/10"
    >
      <div className="grid lg:grid-cols-2">
        <div className="order-2 lg:order-1 p-7 md:p-10">
          ...
          <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl text-foreground tracking-tight leading-[1.05] group-hover:text-primary transition-colors">
            {story.title}
          </h2>

...
async function main() {
  ...
  const [players, articles] = await Promise.all([
    ...
    fetchAll(
      url,
      key,
      "/rest/v1/players?select=slug,updated_at&slug=not.is.null&archived=not.is.true&or=(discovery_method.is.null,verified.is.true,caps_rdc.gt.0)&order=updated_at.desc",
    ),
    fetchAll(
      url,
      key,
